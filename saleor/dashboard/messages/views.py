...
def resend(request, pk=None):
    if request.method == 'POST':
        if pk:
            message = get_object_or_404(Notification, pk=pk)
            # message.mark_as_read()
            sms_response = sendSms(message.to_number, message.description, message.verb, actor=request.user,
...
def write(request):
    if request.method == 'POST':
        ...
        if request.POST.get('single'):
            ...
            if user:
                notif = Notification(to='supplier', actor=request.user, recipient=request.user, sent_to=user.id,
                                     verb=subject, description=body)
                notif.save()
            else:
                notif = Notification(to='anonymous', actor=request.user, recipient=request.user, sent_to=single,
                                     verb=subject, description=body)
                notif.save()
