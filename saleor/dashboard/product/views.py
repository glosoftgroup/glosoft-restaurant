...
def reorder_pagination(request):
    ...
    if p2_sz:
        ...
        low_stock = paginator.page(page)
        return TemplateResponse(request, 'dashboard/re_order/pagination/paginate.html',
                                {"low_stock": low_stock, 'pn': paginator.num_pages})
...
def re_order_form(request, pk):
    ...
    if qset.exists():
        ...
        ctx = {'items': items, 'order': qset, "product": product, 'suppliers': suppliers, 'variants': variants,
...
def search_class_list(request):
    if request.is_ajax():
        ...
        if q is not None:
            ...
            if p2_sz:
                queryset_list = paginator.page(page)
                return TemplateResponse(request, 'dashboard/product/subcategory/paginate.html',
                                        {'classes': product_results})
...
def product_class_create(request, new_window=None):
    ...
    if form.is_valid():
        ...
        if new_window:
            return HttpResponse(
                '<script>function closeme(){ window.opener.refreshProductType(); window.close();} closeme();</script>')
        # return redirect('dashboard:product-class-list')
    ...
    if request.is_ajax():
        ...
        try:
            ...
            if stock_pk in stock_list:
                trail = str(productName) + ' Stock ' + crud + ': quantity ' + str(diff).replace('-',
                                                                                                '') + ': Total stock ' + str(
                    quantity)
            else:
...
def paginate_attr(request):
    ...
    try:
        ...
        try:
            ...
            queryset_list = paginator.page(paginator.num_pages)
        product_results = queryset_list
        return TemplateResponse(request, 'dashboard/product/attributes/pagination/paginate.html',
                                {'attributes': product_results})
    except Exception, e:
        return HttpResponse()
...
def stock_location_pagination(request):
    ...
    if list_sz:
        ...
        if p2_sz:
            ...
            stock_locations = paginator.page(page)
            return TemplateResponse(request, 'dashboard/product/stock_locations/pagination/paginate.html',
...
def stocks(request):
    try:
        ...
        if request.GET.get('initial'):
            ...
            return TemplateResponse(request, 'dashboard/purchase/purchase_list.html',
                                    {'product_results': product_results, 'categories': categories,
                                     'pn': paginator.num_pages})
    except TypeError as e:
        logger.error(e)
        return HttpResponse('error accessing users')
...
def stock_paginate(request):
    ...
    if request.GET.get('gid'):
        ...
        paginator = Paginator(product_results, 10)
        product_results = paginator.page(page)
        return TemplateResponse(request, 'dashboard/purchase/p2.html',
                                {'product_results': product_results, 'pn': paginator.num_pages, 'sz': 10,
        ...
        if list_sz:
            ...
            product_results = paginator.page(page)
            return TemplateResponse(request, 'dashboard/purchase/p2.html',
                                    {'product_results': product_results, 'pn': paginator.num_pages, 'sz': list_sz,
                                     'gid': 0})
        else:
            paginator = Paginator(product_results, 10)
        if p2_sz:
...
def stock_search(request):
    if request.is_ajax():
        ...
        if q is not None:
            ...
            return TemplateResponse(request, 'dashboard/purchase/search.html',
...
def stock_filter(request):
    ...
    ctx = {'pn': paginator.num_pages, 'products_count': products_count, 'product_results': product_results,
           'search_count': len(product_results)}
    return TemplateResponse(
...
def search_productclass(request):
    if request.method == 'POST':
        if request.is_ajax():
            ...
            ctx = {'productClass_count': productClass_count, 'productClass_results': productClass_results,
    ...
    if request.method == 'POST':
        if request.is_ajax():
            ...
            attributes = [
                (attribute.pk, attribute.name, attribute.values.all())
                for attribute in
                ProductAttribute.objects.filter(name__icontains=search).prefetch_related('values').order_by('-id')]
    ...
    if request.is_ajax():
        ...
        if q is not None:
            queryset_list = [
                (attribute.pk, attribute.name, attribute.values.all())
                for attribute in
                ProductAttribute.objects.filter(name__icontains=q).prefetch_related('values').order_by('-id')]
            paginator = Paginator(queryset_list, 10)
...
def add_new_attribute(request, pk=None):
    if request.method == 'POST':
        ...
        return HttpResponse(json.dumps({"value": attribute.id, "text": attribute.name}),
