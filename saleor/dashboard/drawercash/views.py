...
def transactions(request):
    try:
        ...
        return TemplateResponse(request, 'dashboard/cashmovement/transactions.html',
                                {'transactions': transactions, 'pn': paginator.num_pages})
...
def transaction_pagination(request):
    ...
    if p2_sz:
        ...
        transactions = paginator.page(page)
        return TemplateResponse(request, 'dashboard/cashmovement/pagination/paginate.html',
...
def terminals(request):
    try:
        ...
        logger.info('User: ' + str(request.user.name) + ' accessed terminals')
        return TemplateResponse(request, 'dashboard/terminal/terminals.html',
...
def terminal_add(request):
    try:
        ...
        logger.info('User: ' + str(request.user.name) + 'accessed terminal add page')
...
def terminal_process(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            logger.info('User: ' + str(request.user.name) + ' created terminal:' + str(terminal_name))
...
def terminal_detail(request, pk):
    ...
    logger.info('User: ' + str(request.user.name) + ' accessed terminal:' + str(user.terminal_name))
...
def terminal_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        logger.info('User: ' + str(request.user.name) + ' deleted terminal:' + str(terminal.terminal_name))
...
def terminal_edit(request, pk):
    ...
    logger.info('User: ' + str(request.user.name) + ' accessed edit page for user: ' + str(terminal.terminal_name))
...
def terminal_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        logger.info('User: ' + str(request.user.name) + ' updated terminal: ' + str(terminal.terminal_name))
        return HttpResponse("success")
...
def terminal_history(request, pk=None):
    if request.method == 'GET':
        if pk:
            ...
            # logger.info('User: ' + str(request.user.name) + 'accessed terminal history for terminal for:' + str(user.terminal_name))
