...
def groups(request):
    ...
    logger.info('User: ' + str(request.user.name) + ' accessed the view groups page')
...
def group_paginate(request):
    ...
    if list_sz:
        ...
        groups = paginator.page(page)
        return TemplateResponse(request, 'dashboard/permissions/p2.html',
                                {'groups': groups, 'pn': paginator.num_pages, 'sz': list_sz})
    else:
        paginator = Paginator(groups, 5)
    if p2_sz:
...
def group_search(request):
    if request.is_ajax():
        ...
        if q is not None:
            ...
            return TemplateResponse(request, 'dashboard/permissions/search.html',
                                    {'groups': groups, 'pn': paginator.num_pages, 'sz': sz, 'q': q})
        ...
        if login_status == 'inactive':
            ...
            if group_has_permissions in permission_list:
                try:
                    ...
                    logger.info(request.user.name, 'added permissions to group: ' + group.name)
                    return HttpResponse('permissions added')
                ...
                try:
                    ...
                    logger.info(request.user.name, 'updated permissions to group: ' + group.name)
                    return HttpResponse('permissions updated')
                except IntegrityError as e:
                    logger.debug(e)
                    logger.error(e)
