...
def option_searchs(request):
    if request.is_ajax():
        ...
        if q is not None:
            ...
            if p2_sz:
                options = paginator.page(page)
                return TemplateResponse(request, 'dashboard/payment/options/paginate.html',
                                        {'options': options, 'sz': sz})
...
def options_paginate(request):
    ...
    if request.GET.get('gid'):
        ...
        if list_sz:
            ...
            options = paginator.page(page)
            return TemplateResponse(request, 'dashboard/payment/options/p2.html',
