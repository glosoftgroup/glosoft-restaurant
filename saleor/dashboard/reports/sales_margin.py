    ...
    try:
        ...
        for sale in all_sales:
            ...
            for i in SoldItem.objects.filter(sales=sale):
                try:
                    total_cost_price = i.unit_purchase * i.quantity
                except:
                    total_cost_price = 0

                costPrice.append(total_cost_price)
...
def sales_detail(request, pk=None):
    try:
        ...
        for t in items:
            try:
                itemPrice = t.unit_purchase * t.quantity
            except:
                itemPrice = 0
        ...
        if list_sz:
            ...
            return TemplateResponse(request, 'dashboard/reports/sales_margin2/items/p2.html',
                                    {'items': total_items, 'pn': paginator.num_pages, 'sz': list_sz, 'gid': 0,
                                     'total_sales': total_sales, 'tsum': tsum})
        else:
