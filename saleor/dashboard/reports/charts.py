...
from .hours_chart import (
    get_item_results, get_terminal_results,
    get_user_results, get_hours_results,
    ...
    if date:
        try:
            if point and point != 'all':
                sales_by_category = SoldItem.objects.filter(sales__created__contains=date,
                                                            sale_point__name__contains=point).values(
...
def sales_date_chart(request):
    ...
    if date:
        try:
            ...
            try:
                drawerdeposit = \
                DrawerCash.objects.filter(created__contains=date, trans_type__icontains='deposit').aggregate(
            ...
            try:
                drawerwithdraw = \
                DrawerCash.objects.filter(created__contains=date, trans_type__icontains='withdraw').aggregate(
                    Sum('amount'))['amount__sum']
            ...
            try:
                drawersales = DrawerCash.objects.filter(created__contains=date, trans_type__icontains='sale').aggregate(
                    Sum('amount'))['amount__sum']
...
def get_sales_by_week(request):
    ...
    for i in range(7):
        amount = get_date_results_range(DateFormat(i * drf2 + first_range_date).format('Y-m-d'),
                                        DateFormat((i + 1) * drf2 + first_range_date).format('Y-m-d'))
        r = str((DateFormat(i * drf + first_range_date).format('d/m'))) + ' - ' + str(
            DateFormat((i + 1) * drf + first_range_date).format('d/m'))
        labels3.append(r)
    ...
    try:
        ...
        date_total_discount = \
        Sales.objects.filter(created__range=[date_from, date_to]).aggregate(Sum('discount_amount'))[
            'discount_amount__sum']
            ...
            if point and point != 'all':
                users = Sales.objects.values('user__email', 'user__name', 'user').annotate(Count('user')).annotate(
                    ...
                    Sum('solditems__quantity')).order_by('-solditems__quantity__sum').filter(created__contains=date,
                                                                                             sale_point__name__contains=point)
                point = SalePoint.objects.get(name=point)
                point_name = point.name
            else:
                users = Sales.objects.values('user__email', 'user__name', 'user').annotate(Count('user')).annotate(
    ...
    if date:
        try:
            ...
            if point and point != 'all':
                ...
                point = point
                point_name = point
            sales_by_category_totals = terminals.aggregate(Sum('solditems__quantity__sum'))[
                'solditems__quantity__sum__sum']
            new_sales = []
            for sales in terminals:
                color = "#%03x" % random.randint(0, 0xFFF)
                sales['color'] = color
...
def sales_discount_chart(request):
    ...
    if image:
        ...
        if point and point != 'all':
            sales_by_category = SoldItem.objects.filter(sales__created__contains=date,
                                                        sale_point__name__contains=point).filter(
                ~Q(discount=0.00)).values(
                'product_category').annotate(
        ...
        try:
            ...
            for sales in sales_by_category:
                ...
                for s in range(0, sales_by_category.count(), 1):
                    sales['count'] = s
                new_sales.append(sales)
            categs = SoldItem.objects.values('product_name', 'discount').annotate(
                Count('product_name', distinct=True)).order_by()
            this_year = today.year
            avg_m = Sales.objects.filter(created__year=this_year).filter(~Q(discount_amount=0.00)).annotate(
                c=Count('total_net'))
            highest_category_sales = new_sales[0]['product_name']
            default = []
            labels = []
