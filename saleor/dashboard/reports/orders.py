    ...
    if date:
        try:
            ...
            if p2_sz and date:
                ...
                orders = paginator.page(page)
                return TemplateResponse(request, 'dashboard/reports/orders/paginate.html',
                                        {'point_pk': point_pk, 'orders': orders, 'gid': date})

            paginator = Paginator(orders, 10)
            orders = paginator.page(page)
            return TemplateResponse(request, 'dashboard/reports/orders/p2.html',
                                    {'point': point, 'point_pk': point_pk, 'orders': orders, 'pn': paginator.num_pages,
                                     'sz': 10, 'gid': date,
        ...
        try:
            ...
            if p2_sz:
                ...
                orders = paginator.page(page)
                return TemplateResponse(request, 'dashboard/reports/orders/paginate.html',
                                        {'point_pk': point_pk, 'orders': orders})
            ...
            if request.GET.get('gid'):
                ...
                if list_sz:
                    ...
                    return TemplateResponse(request, 'dashboard/reports/orders/search.html',
                                            {'point_pk': point_pk, 'point': point, 'orders': orders,
