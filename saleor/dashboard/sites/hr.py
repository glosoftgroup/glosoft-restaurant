...
from ...hr.models import Bank, BankBranch, UserRole, Department
...
def view_roles(request):
    try:
        ...
        if request.GET.get('initial'):
            ...
            return TemplateResponse(request, 'dashboard/sites/hr/roles/view.html',
                                    {'roles': roles, 'totalp': paginator.num_pages})
...
def role_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            role.save()
            user_trail(request.user.name, 'updated user role from: ' + str(role.name) + ' to: ' + str(new_role),
                       'update')
            return HttpResponse('success')
        except:
            HttpResponse('error')
