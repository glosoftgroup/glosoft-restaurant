...
def view(request, pk=None):
    try:
        ...
        logger.info('User: ' + str(request.user.name) + 'accessed the branches page for bank: ' + bank.name, 'view')
        if request.GET.get('initial'):
            return HttpResponse(paginator.num_pages)
        else:
            return TemplateResponse(request, 'dashboard/sites/hr/bank/branch/view.html',
                                    {'branches': branches, 'bank': bank, 'totalp': paginator.num_pages})
...
def search(request):
    ...
    if q is not None:
        ...
        if p2_sz:
            branches = paginator.page(page)
            return TemplateResponse(request, 'dashboard/sites/hr/bank/branch/paginate.html',
                                    {'branches': branches, 'bank': bank})
